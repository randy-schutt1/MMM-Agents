...
before the click, which is item 188's proposed fix and **is why `§8a` measured zero** (item 197, the
...
**The reviewer should treat them as a sample and re-check the other twenty-three** (item 199).
...
only reason this was caught. **It is the inverse of V16's item 199**: V16 let a transcript contaminate
frame names; V17 let frames contaminate the reading of a transcript. **Same root — one channel
adjudicating another with no independent third.** Item **202**.
...
two full runs are now byte-identical. Item **208**.
...
### QUARANTINE — `Q-018`, AND ITEM 200'S PREDICTION IS FALSIFIED

Item 200 predicted the generator had *"moved to paraphrasing"* so *"a byte `diff` will not flag
...
lands by coincidence**, which is why the audit is on provenance, never plausibility. Item **201**.

### ITEM 198's REMEDY, TESTED — AND A NEGATIVE RESULT
...
picture. Item 198 narrows from *"do it"* to *"check the image type first"*. ⭐⭐ **And §8a on that
second sweep came back `−1`, not `0`** — same script, same machine, forty minutes apart — which is
§8a's own *"it varies per run"* warning demonstrated rather than quoted. Item **205**.
...
**REVIEW_INDEX items 201–208.** ⚠ **Nineteen opened against eight advanced is the finding:** V17 is
...
No `I-009` collision. No policy ledger edited (`D-038a`) — which is why items 197, 205 and 207 are
