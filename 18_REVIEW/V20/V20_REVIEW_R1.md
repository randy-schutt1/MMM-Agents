...
⛔ **`V21`'s GATE IS CLOSED** under `DECISIONS.md` **D-024** — one `MAJOR`. It opens when action 1
lands **and is re-reviewed** (`D-003`).

**The `MAJOR` is `M1`, and it is not the defect the submission flagged.** The submission correctly
identified that `PT-048`'s baseline was never wired into its verdict, disclosed it everywhere, and
put it to the reviewer. **That is handled well and is charged as a `MINOR` for the record only.**
What this round found underneath it is different: **the runner does not implement `PT-048`'s primary
measure as the pre-registration defines it, the substitution biases the statistic toward the claimed
band, and correcting it flips the primary verdict from `CONFIRMED` to `REFUTED`.** `BT_V20_0001.md`
§6 states there was *"no other disagreement"* between runner and pre-registration. There was, and it
is the one that decided the result.
...
⚠️ **And note the consequence of this round's own `MAJOR`:** V21's gate is now closed on V20's
...
| **Independent ASR** | ⭐ **A full-file `faster-whisper` / CTranslate2 `large-v3` pass was run BY THIS REVIEWER** — the channel the submission declares it did not run (§2 below) |
...
## §3 — THE MISSING ASR PASS — THIS REVIEWER RAN ONE

The submission declares (mastery §2.2, and its own item **326**): *"NO INDEPENDENT ASR PASS WAS RUN —
...
`faster-whisper` `large-v3` over all 45:49 — and tested what the gap actually cost.

*(§3.1 records the result.)*
...
| # | Item |
|---|---|
| **333** | ⚠️ **`PT-048`'s STANDING VERDICT MUST BE RECORDED AS A NULL — THE SUBMISSION'S OWN ITEM 320, RULED ON.** §4 defines `N1` and states the null in its own words; §5's table never references it; the baseline returns **23.0** against the legs' **22.45**, on the far side of the point estimate. The submission disclosed this everywhere, downgraded the class to `DESCRIPTIVE`, and asked the reviewer to rule. **Ruling: the standing result is a NULL.** `MINOR` rather than `MAJOR` because **the word never travelled unqualified** — every mention outside `BT_V20_0001.md` carries the null in the same sentence. `E25`-adjacent, but the negative result was retained, not omitted |
| **334** | ⚠️ **THE MISSING INDEPENDENT ASR PASS — THE SUBMISSION'S OWN ITEM 326, RULED ON.** No pass of any kind was run; the transcript corrections rest on the printed deck plus internal consistency. ⭐ **The deck is Tier 2 and the ASR would have been Tier 3, so the four corrections V20 actually makes are better evidenced than an ASR could have made them** — the exposure is what was *not* looked for. **This reviewer ran the full-file pass the submission owed** (§3.1). Charged `MINOR` as a process/completeness gap that is now discharged for V20 but must not become the norm |
| **335** | *(see §3.1 — allocated to the ASR sweep's finding)* |
...
| **341** | ⭐ **THE COUNT DISCIPLINE ASKED FOR BY ITEMS 249 / 293 / 317 IS BEING APPLIED AND IS CORRECT.** `shift his own` ×5 re-derived at the five named markers (the fifth is the plural form); `RSI` = 1 correctly explained as the substring in *"Unive**RSI**ty"*; `handle` = 7 with only 4 being the word |
...
1. ⛔ **Item 332 (`MAJOR`).** Change `run_pt048.py`'s `P1` interior counter-swing detector to use
   ...
   **Correct §6's *"No other disagreement"* sentence** to record this divergence — the
   pre-registration governs and is **not** edited. **State the resulting verdict on the corrected
   primary cell.** If the implementer maintains that `±1` is a defensible reading of §3, then report
   **both** scales side by side and let §5 be applied to the pre-registration's own `±3`, with the
   sensitivity shown as `N3`-style bracketing. **This round's figures are in
   `06_MANUAL_BACKTEST/scripts/rev_pt048_independent.py` and are re-runnable.**
2. **Item 333.** Record `PT-048`'s standing result as a **NULL** in `BT_V20_0001.md` §0,
...
3. **Item 334.** No action for V20 — the pass was run in this round and is reported at §3.1. **The
   requirement stands for V21.**

**Not required:** items 344–346 and the §5 decision-table hole (345) are for the successor
...
MAJOR ISSUES:
- M1  E18  PT-048's primary measure P1 is computed with a +/-1 interior swing
           detector while PT-048 §3.1 defines a swing pivot at +/-3. Correcting
           it moves the primary median from 22.40 [22.00,23.20] to
           30.10 [28.70,31.25] and flips the verdict CONFIRMED -> REFUTED. The
           substitution also inflates n from 706 to 2,519, narrowing the very
           interval §5 made a condition. BT_V20_0001.md §6 records "no other
           disagreement" and cites PIVOT_K=3 as honoured.

REQUIRED ACTIONS:
1. Fix the P1 detector to use PIVOT_K, re-run, republish BT_V20_0001.md §§0/2/3,
   and correct §6. Do not edit the pre-registration.                     (332)
2. Record PT-048's standing result as a NULL in four files.              (333)
3. No action - the owed ASR pass was run in this round.                  (334)

ADVANCEMENT: NOT AUTHORIZED
```

⛔ **`V21`'s GATE IS CLOSED** under `D-024` — one `MAJOR`. It opens when action 1 lands **and is
re-reviewed**. ⭐ **Action 1 changes one constant and re-runs a 90-second script**; actions 2–3 are
bookkeeping. **This is a short remediation round, not a re-test from scratch.**

**V20 STATUS: `IN REMEDIATION`.** Not `COMPLETE` until items 332–334 are applied and independently
verified.
