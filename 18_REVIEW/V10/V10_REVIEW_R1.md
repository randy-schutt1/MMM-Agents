...
table is labelled as *measured examples, not the library's constant*; the prose instruction
becomes **read `frameRate` from the header and patch `rate × 10`**; the worked example carries
V10's 2.0 as a second data point; and the speedup table's control column is relabelled. The
sweep snippet gains the derived-speed computation. **See open item 87 (formerly 82), now
`CLOSED`.**
