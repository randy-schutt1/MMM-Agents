...
| 35 | Same | **Owner owes two decisions before the first `BT_` file exists**: (a) manual-phase development/holdout boundary, (b) baseline parameters — iterations, eligible window, direction handling. `validate_project.py` **fails** while `DECISIONS.md` still lists these as `OWED NOW`, so this is mechanically blocking, not advisory | `DECISIONS.md` open-decisions table | OPEN — owner action |
---
