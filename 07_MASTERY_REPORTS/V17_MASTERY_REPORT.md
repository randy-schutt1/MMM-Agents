...
`§6` warned that items 236–243 (allocated as 201–208) might be renumbered on integration. **They must be, and so must
...
| # | What | Where declared |
|---|---|---|
| `D1` | ⚠ **`SWF_CAPTURE_RECIPE.md` §9's transcript-before-frames ordering was BROKEN** — fifth consecutive lesson. Mitigated by `[AUDIO]`/`[PRINTED]`/`[VISUAL]` tags, which are `grep`-falsifiable. ⚠ **And `S1` below shows the cost is real, not ceremonial** | `V17_SOURCE_NOTES.md` §0; item **241** |
...
| `D3` | ⚠ **The frame sweep DID NOT follow §10 literally** — `t0` before the click (item 188's fix, V16's `D3`). The recipe file was **NOT** edited (`D-038a`, policy ledger) | `04_SCREENSHOTS/V17/INDEX.md` §0.2; item **218**, now **twice**-evidenced |
| `D4` | **No forward read.** V18+ not opened. V16's committed artifacts were read — a **backward** read | `V17_SOURCE_NOTES.md` §0 |
| `D5` | ⚠ **The §7 detector under-samples short-lived screens**; the quiz's answer chart survived on **one** frame below the initial threshold | `INDEX.md` §0.4; item **242** |
...
| `D7` | ⚠ **`run_pt045.py` was EXECUTED BEFORE IT WAS COMMITTED**, departing from the `PT-044` precedent (`cc6d04e`, *"committed BEFORE execution"*). **No written rule in `COMMON_PROTOCOL.md` §9 requires it** — but the precedent existed and was not followed. **The pre-registration was committed first (`7eaf4d1`) and was never edited** | `BT_V17_0001.md` §6; item **243** |
...
8. **`PT-045`:** the daily wick's **size** is indistinguishable from a shuffled version of the same
   ...
   about market makers.**
9. **`Q-018`, and item 221's forward prediction is falsified** — the trio is a **byte clone** of
...
`REVIEW_INDEX.md` items **236–243** (allocated as **201–208**). ⚠ **If a concurrent session has allocated these numbers against
...
| Item | Subject |
|---|---|
| **236** | ⭐⭐ Item 221's forward prediction **falsified** — V17's trio is a **byte clone**; fix the detector on the invariant |
| **237** | ⚠⚠ **`S1` self-charge** — a pre-committed check overturned this session's headline claim; the **inverse** of item 220's failure |
| **238** | ⭐ V17 dates and numbers itself, and *"actually nine"* bears on `A-01`'s missing Week 6 |
| **239** | ⚠⚠ **Arm B excludes ~25% of days to arm A's 1–8%** — reaches past `PT-045` into every test |
| **240** | ⭐ Item 219's remedy **tested and negative** for slide-embedded charts; and §8a came back `−1` on a second sweep |
| **241** | ⚠ §9's ordering broken for the **fifth** consecutive lesson, and item 237 is what §9 exists to prevent |
| **242** | ⚠ The §7 detector can miss a screen entirely; the quiz's answer chart survived on one sub-threshold frame |
| **243** | ⚠ `PT-045`'s two process defects, and its §6 decision table has a hole |
...
* **No `I-009` collision.** V17 ran in a dedicated worktree on branch `video/v17` under `D-038`,
  ...
  edited** — `SWF_CAPTURE_RECIPE.md` is untouched, which is why items 218, 240 and 242 are deferred
  rather than applied.
